...
def main(cfg: DictConfig):
    ...
    if "hyper" in method:
        ...
        # latent space shape: [2100 (n_instances), 4 (#ch), 32(res), 96(res*3)]
        x = pretrained_weights['weights_latent_space'].permute(0, 2, 3, 1)
        # latent space shape: [2100 (n_instances), 32(res), 96(res*3), 4 (#ch)]
        n, H, W, C = x.shape
        patch_h, patch_w = 16, 16
        # reshape into patch grid
        x = x.reshape(n, H // patch_h, patch_h, W // patch_w, patch_w, C)
        # permute to put patches together
        x = x.permute(0, 1, 3, 2, 4, 5)
        # flatten patch grid
        x = x.reshape(n, (H // patch_h) * (W // patch_w), patch_h * patch_w * C)
        layers = []
        layer_names = []
        for i in range(x.shape[1]):
            layers.append(x.shape[2])
            layer_names.append(f"layer_{i}")
        # import pdb; pdb.set_trace()
        reshaped_pretrained_weights = x
        model = Transformer(
